...
carteira2 = "S221015197"  # Código da segunda carteira S que deseja consultar
carteira3 = "S321015197"  # Código da terceira carteira S que deseja consultar
carteira4 = "S328252097"  # Código da terceira carteira S que deseja consultar
carteira5 = "S521015197"  # Código da terceira carteira S que deseja consultar
carteira6 = "S925754197"  # Código da terceira carteira S que deseja consultar

# Multiplicadores para cada carteira
multiplicador_carteira1 = 15  # Substitua pelo multiplicador desejado para a carteira 1
...
multiplicador_carteira4 = 20  # Substitua pelo multiplicador desejado para a carteira 4
multiplicador_carteira5 = 1  # Substitua pelo multiplicador desejado para a carteira 5
multiplicador_carteira6 = 1  # Substitua pelo multiplicador desejado para a carteira 6

# Intervalo de datas
data_inicial = datetime(2023, 1, 2)  # Data inicial no formato AAAA, Mês, Dia
data_final = datetime(2023, 12, 12)    # Data final no formato AAAA, Mês, Dia

# Função para somar os valores de máxima, mínima e fechamento
def somar_maxima_minima_fechamento(df1, df2, df3, df4, df5, df6):
    # Somar os valores de máxima, mínima e fechamento para as carteiras após multiplicá-los
    soma_maxima = (df1["high"] * multiplicador_carteira1) + (df2["high"] * multiplicador_carteira2) + (df3["high"] * multiplicador_carteira3) + (df4["high"] * multiplicador_carteira4) + (df5["high"] * multiplicador_carteira5) + (df6["high"] * multiplicador_carteira6)
    soma_minima = (df1["low"]  * multiplicador_carteira1) + (df2["low"] * multiplicador_carteira2) + (df3["low"] * multiplicador_carteira3) + (df4["low"] * multiplicador_carteira4) + (df5["low"] * multiplicador_carteira5) + (df6["low"] * multiplicador_carteira6)
    soma_fechamento = (df1["close"]  * multiplicador_carteira1) + (df2["close"] * multiplicador_carteira2) + (df3["close"] * multiplicador_carteira3) + (df4["close"] * multiplicador_carteira4) + (df5["close"] * multiplicador_carteira5) + (df6["close"] * multiplicador_carteira6)
...
while data_atual <= data_final:
    ...
    if data_atual.weekday() not in [5, 6]:
        ...
        soma_maxima, soma_minima, soma_fechamento = somar_maxima_minima_fechamento(bars_df_carteira1, bars_df_carteira2, bars_df_carteira3, bars_df_carteira4, bars_df_carteira5, bars_df_carteira6)
...
resultados_df.to_csv(f'Relatorios/Max_Min_Fech_Carteiras_{carteira1}x{multiplicador_carteira1}_{carteira2}x{multiplicador_carteira2}_{carteira3}x{multiplicador_carteira3}_{carteira4}x{multiplicador_carteira4}_{carteira5}x{multiplicador_carteira5}_{carteira6}x{multiplicador_carteira6}_{data_inicial.strftime("%Y%m%d")}_{data_final.strftime("%Y%m%d")}_CSV.csv', index=False)
resultados_df.to_excel(f'Relatorios/Max_Min_Fech_Carteiras_{carteira1}x{multiplicador_carteira1}_{carteira2}x{multiplicador_carteira2}_{carteira3}x{multiplicador_carteira3}_{carteira4}x{multiplicador_carteira4}_{carteira5}x{multiplicador_carteira5}_{carteira6}x{multiplicador_carteira6}_{data_inicial.strftime("%Y%m%d")}_{data_final.strftime("%Y%m%d")}_Excel.xlsx', index=False)
