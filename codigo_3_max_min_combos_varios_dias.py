# CÓDIGO QUE PERCORRE UM INTERVALO DE DATA E GERA 1 ÚNICO ARQUIVO COM A MÁXIMA E MÍNIMA DE CADA DIA DE UMA COMBINAÇÃO DE 2 CARTEIRAS, JÁ PULANDO OS SÁBADOS E DOMINGOS DO MÊS
...
# Intervalo de datas
data_inicial = datetime(2023, 10, 1)  # Data inicial no formato AAAA, Mês, Dia
data_final = datetime(2023, 10, 9)    # Data final no formato AAAA, Mês, Dia

# Função para somar os valores de máxima e mínima
def somar_maxima_minima(df1, df2):
    soma_maxima = df1["high"] + df2["high"]
    soma_minima = df1["low"] + df2["low"]
...
resultados_df.to_csv(f'Relatorios/Maior_Max_Min_Carteiras_{carteira1}_{carteira2}_{data_inicial.strftime("%Y%m%d")}_{data_final.strftime("%Y%m%d")}_CSV.csv', index=False)
resultados_df.to_excel(f'Relatorios/Maior_Max_Min_Carteiras_{carteira1}_{carteira2}_{data_inicial.strftime("%Y%m%d")}_{data_final.strftime("%Y%m%d")}_Excel.xlsx', index=False)
