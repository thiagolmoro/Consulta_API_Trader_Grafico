# CÓDIGO QUE PERCORRE UM INTERVALO DE DATA E GERA 1 ARQUIVO PARA CADA DIA DENTRO DO INTERVALO JÁ PULANDO OS SÁBADOS E DOMINGOS DO MÊS
...
while data_atual <= data_final:
    ...
    if data_atual.month == 9 and data_atual.weekday() not in [5, 6]:  # Verifica se o mês é setembro e não é sábado ou domingo
        ...
        if requisicao.status_code == 200:
            ...
            bars_df.to_csv(f'Relatorios/Carteira_{carteira}_{data_intraday}_CSV.csv')
            bars_df.to_excel(f'Relatorios/Carteira_{carteira}_{data_intraday}_Excel.xlsx')
